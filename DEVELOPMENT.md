...
```
git clone git@github.com:enigmacurry/rust-acme-dns-client.git \
  ~/git/vendor/enigmacurry/rust-acme-dns-client
cd ~/git/vendor/enigmacurry/rust-acme-dns-client
```
...
## Build and run development rust-acme-dns-client
...
alias rust-acme-dns-client='just -f ~/git/vendor/enigmacurry/rust-acme-dns-client/Justfile run'
source <(rust-acme-dns-client completions bash 2> /dev/null)
```

Now you can run `rust-acme-dns-client` from any directory, with
...
report](https://enigmacurry.github.io/rust-acme-dns-client/coverage/master/).
